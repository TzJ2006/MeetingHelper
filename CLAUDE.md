...
python3 scripts/local_llm.py transcripts/2026-07-07.txt "Summarize action items."
```
...
- `scripts/hf_asr.py` — optional Hugging Face ASR worker (`--asr hf`). Swift streams NDJSON audio frames to it over stdin and reads NDJSON transcript events back.
- `scripts/sherpa_asr.py` — optional sherpa-onnx bilingual ASR worker (`--asr sherpa`), same NDJSON protocol. Deps installed with `python3 -m pip install --target .build/pydeps sherpa-onnx`.
- `scripts/local_llm.py` — standalone helper; reads transcript files and calls an OpenAI-compatible local API (`LOCAL_LLM_BASE_URL`/`LOCAL_LLM_MODEL`/`LOCAL_LLM_API_KEY`).
